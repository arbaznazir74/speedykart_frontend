...
export default function HotBoxBannersPage() {
  const crud = useCrud<Banner>({
    endpoint: `${API_ENDPOINTS.BANNER}?platformType=${PlatformType.HotBox}`,
    writeEndpoint: API_ENDPOINTS.BANNER,
  });
  return (
    <CrudPage title="HotBox Banners" description="Manage HotBox promotional banners" columns={columns} formFields={formFields}
      ...
      onCreate={async (d) => { await crud.create({ ...d, platformType: PlatformType.HotBox } as Partial<Banner>); }}
      onUpdate={async (d) => { await crud.update({ ...d, platformType: PlatformType.HotBox } as Partial<Banner>); }}
