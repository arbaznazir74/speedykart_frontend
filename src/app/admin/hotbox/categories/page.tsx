...
export default function HotBoxCategoriesPage() {
  ...
  const loadPending = useCallback(async () => {
    ...
    try {
      ...
      setPendingCategories(all.filter((c) => String(c.platform) === "1" || String(c.platform) === "HotBox"));
  ...
  return (
    <>
                  ...
                  <div key={pc.id} className="flex items-center justify-between rounded-lg border border-amber-200 bg-white px-4 py-2.5">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant="secondary" className="text-xs">
                        {pc.platform === 1 ? "HotBox" : "SpeedyMart"}
                      </Badge>
      ...
      <CrudPage title="HotBox Categories" description="Manage HotBox food categories" columns={columns} formFields={formFields}
        ...
        onCreate={async (d) => { await crud.create({ ...d, platform: 1, level: 1 } as unknown as Partial<Category>); }}
