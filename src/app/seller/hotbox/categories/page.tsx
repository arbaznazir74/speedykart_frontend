...
export default function SellerHotBoxCategoriesPage() {
  const { user } = useAuth();
  ...
  const loadCategories = useCallback(async (p = 1) => {
    ...
    try {
      const [listR, countR] = await Promise.all([
        apiGet<SellerCategory[]>(`${API_ENDPOINTS.CATEGORY}/parent/seller?platform=1&sellerId=${sellerId}`, { skip, top: pageSize }),
        apiGet<{ intResponse: number }>(`${API_ENDPOINTS.CATEGORY}/parent/seller/count?platform=1&sellerId=${sellerId}`),
      ]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="HotBox Categories" description="Browse food categories. You can suggest a new category for admin approval.">
        ...
        <DialogContent className="max-w-sm">
          ...
          <form onSubmit={handleSuggest} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input required placeholder="e.g. Biryani" value={suggestName} onChange={(e) => setSuggestName(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label>Slug</Label>
